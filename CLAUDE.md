...
1. **Game name** (string) — used for lookup in online databases (GameBase64, CSDb, Lemon64) and to scope the research.
2. **Research question** (string) — the concrete thing the user wants to know.
3. **Memory dump** (binary file, typically 64 KB) — a snapshot of `$0000–$FFFF` from a running VICE session.
4. *(Optional)* **Partial disassembly** (text/asm file) — human-annotated labels, comments, or known routines the user already mapped.
...
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | -------------------------------- |
| **Planner**     | Decomposes the research question into an ordered list of investigative steps (tool calls + hypotheses to test). Replans when the critic rejects.                | question, KB, prior critique   | `plan: List[Step]`               |
| **Executor**    | Picks the next unfinished step and dispatches the right tool (VICE MCP, Capstone, Tavily, or KB lookup). Handles retries and tool errors.                       | plan, KB                       | `tool_results: List[ToolResult]` |
| **Synthesizer** | Folds raw tool outputs into the working KB — normalizes addresses, labels, x-refs, and prose notes. This is the only agent that mutates long-term memory.       | tool_results, KB               | KB updates                       |
| **Analyst**     | Interprets the updated KB against the original question. Produces a candidate answer with cited evidence and a confidence score.                                | KB, question                   | `candidate_answer`               |
| **Critic**      | Adversarial check. Asks: is the answer actually supported? Are there alternative explanations? Is more evidence needed? Decides `accept` / `revise` / `replan`. | candidate_answer, KB, question | `verdict, critique`              |
...
1. `**kb.json`** — append-only event log of every observation (timestamp, source, address range, content, agent that produced it). Source of truth for replay.
2. `**kb.sqlite**` — derived, queryable view with tables:
  ...
  - `hypotheses(id, text, status, evidence_event_ids)p`
...
```python
def main(game: str, question: str, dump_path: Path, partial_asm: Path | None):
